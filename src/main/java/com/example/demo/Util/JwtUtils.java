...
public class JwtUtils {
    ...
    public String createToken(Authentication authentication){
        ...
        String jwtToken = JWT.create()
                ...
                .withExpiresAt(new Date(System.currentTimeMillis() + 1800000)) //Fecha y hora de expiración del token. 30 min despues
                .withJWTId(UUID.randomUUID().toString()) //Identificador único al token
                .withNotBefore(new Date(System.currentTimeMillis())) //Define la fecha y hora antes de la cual el token no es válido.
    ...
    public DecodedJWT validateToken(String token){
        try {
            ...
            //Crea una instancia de Algorithm usando el algoritmo HMAC256. con la clave secreta
            Algorithm algorithm = Algorithm.HMAC256(this.privateKey);


            JWTVerifier verifier = JWT.require(algorithm) //Inicia la configuración de un verificador de token JWT utilizando el algoritmo especificado
                    .withIssuer(this.userGenerator)  //Especifica el emisor esperado del token
                    .build(); //Construye el verificador de token (JWTVerifier) con las configuraciones proporcionada
